...
class User(Base):
    ...
    games_as_player1 = relationship(
        "Game", foreign_keys="Game.player1", back_populates="player1_user"
    )

    games_as_player2 = relationship(
        "Game", foreign_keys="Game.player2", back_populates="player2_user"
    )
...
class Game(Base):
    ...
    player1_user = relationship(
        "User", foreign_keys=[player1], back_populates="games_as_player1"
    )

    player2_user = relationship(
        "User", foreign_keys=[player2], back_populates="games_as_player2"
    )

    current_turn_user = relationship("User", foreign_keys=[current_turn])

    winner_user = relationship("User", foreign_keys=[winner])
